...
class ChunkConfig:
    """Configuration for dynamic chunk sizing and overlap."""
...
def _slice_lines(lines: list[str], start_line: int, end_line: int) -> str:
    """Slice text by 1-based line numbers to preserve exact boundaries."""
...
def _unit_id(path: Path, start_line: int, end_line: int, kind_or_symbol: str) -> str:
    """Deterministic unit ID based on path, line span, and symbol/kind."""
...
def _find_header_end(lines: list[str], config: ChunkConfig, boundary_line: Optional[int]) -> int:
    """Find the end of the header unit using boundary hints and a size budget."""
    if not lines:
        return 0
...
def _detect_boundary(lines: list[str]) -> Optional[int]:
    """Find the first structural boundary to cap the header unit."""
...
def _structural_blocks_from_headings(lines: list[str], start_line: int) -> list[dict]:
    """Infer section blocks from Markdown-style headings."""
    headings = []
...
def _structural_blocks_from_keys(lines: list[str], start_line: int) -> list[dict]:
    """Infer blocks from top-level key/value config lines."""
    keys = []
    for idx in range(start_line, len(lines) + 1):
        line = lines[idx - 1]
        if line.startswith(" "):
...
def _structural_blocks(path: Path, lines: list[str], start_line: int) -> list[dict]:
    """Detect natural structural blocks using available heuristics."""
...
def _fill_gaps(blocks: list[dict], start_line: int, end_line: int) -> list[dict]:
    """Ensure the entire file is covered by blocks (fill gaps with chunks)."""
...
def _split_large_unit(
    ...
    """Split oversized units at good boundaries with overlap."""
    ...
    while start <= end:
        ...
        for line_no in range(start, end + 1):
            ...
            if chars + line_len > config.max_chars:
                # Prefer splitting at a boundary we saw inside the budget.
                if last_boundary and last_boundary >= start:
                    split_end = last_boundary
                else:
                    # Ensure progress even if a single line exceeds the budget.
...
def _merge_small_units(
    ...
    """Merge undersized units with neighbors while respecting size limits."""
    merged: list[dict] = []
    idx = 0
    while idx < len(units):
        unit = units[idx]
        ...
        if merged and merged[-1]["kind"] != "file_header":
            # First try merging with the previous unit to keep order stable.
            prev = merged[-1]
...
def chunk_text(path: Path, text: str, config: Optional[ChunkConfig] = None) -> list[dict]:
    ...
    Flow:
    1) emit a file_header unit for top-of-file context
    2) detect structural blocks (defs/headings/keys) and fill gaps
    3) split oversized blocks, then merge undersized ones
    """
    if config is None:
        config = ChunkConfig()
